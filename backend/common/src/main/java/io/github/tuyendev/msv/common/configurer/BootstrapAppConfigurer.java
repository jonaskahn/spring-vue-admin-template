...
public class BootstrapAppConfigurer {
	...
	private static boolean isNecessaryUpdateAuthority(AuthorityType authorityType, Authority found) {
		if (!Objects.equals(authorityType.value(), found.getName())) {
			return true;
		}
		return !Objects.equals(authorityType.desc(), found.getDescription());
	}
	...
	private void createDefaultAuthorities() {
		Map<String, Authority> previousAuthorities = StreamEx.of(authorityRepo.findByNameIsIn(AuthorityType.names()))
				.toMap(Authority::getName, Function.identity());
		for (AuthorityType auth : AuthorityType.values()) {
			Authority found = previousAuthorities.get(auth.value());
