            ...
            <div class="card">
                ...
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            ...
                            <Button :disabled="!selectedProducts || !selectedProducts.length" class="p-button-danger" icon="pi pi-trash" label="Delete" @click="confirmDeleteSelected" />
                        </div>
                    </template>

                    <template v-slot:end>
                        <FileUpload :maxFileSize="1000000" accept="image/*" chooseLabel="Import" class="mr-2 inline-block" label="Import" mode="basic" />
                ...
                >
                    ...
                    <Column headerStyle="min-width:10rem;">
                        <template #body="slotProps">
                            <Button class="p-button-rounded p-button-success mr-2" icon="pi pi-pencil" @click="editProduct(slotProps.data)" />
                            <Button class="p-button-rounded p-button-warning mt-2" icon="pi pi-trash" @click="confirmDeleteProduct(slotProps.data)" />
                        </template>
                    </Column>
                </DataTable>

                <Dialog v-model:visible="productDialog" :modal="true" :style="{ width: '450px' }" class="p-fluid" header="Product Details">
                    <img v-if="product.image" :alt="product.image" :src="contextPath + 'demo/images/product/' + product.image" class="mt-0 mx-auto mb-5 block shadow-2" width="150" />
                    <div class="field">
                        <label for="name">Name</label>
                        <InputText id="name" v-model.trim="product.name" :class="{ 'p-invalid': submitted && !product.name }" autofocus required="true" />
                    ...
                    <div class="formgrid grid">
                        <div class="field col">
                            ...
                            <InputNumber id="price" v-model="product.price" :class="{ 'p-invalid': submitted && !product.price }" :required="true" currency="USD" locale="en-US" mode="currency" />
